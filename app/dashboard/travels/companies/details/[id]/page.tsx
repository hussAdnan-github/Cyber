import { Building2, Phone, MapPin, AlertCircle, Mail, Map } from "lucide-react";
...
export default async function CompanyDetailsPage({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (response.data && response.data.success) {
      ...
      errorMessage = response.data?.message || "فشل في جلب تفاصيل الشركة";
    }
  } catch (error: any) {
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div className="flex gap-2 order-2 md:order-1 mt-4 md:mt-0">
           <Link href={`/dashboard/travels/companies/${company.id}`} className="bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 px-6 py-2.5 rounded-lg text-sm font-bold transition-colors shadow-sm flex items-center">
             تعديل البيانات
           </Link>
        </div>
        ...
        <div className="text-right order-1 md:order-2 ml-auto">
           ...
           <div className="flex items-center gap-2 text-gray-500 text-xs justify-end">
              <span className="text-gray-400">السفريات</span>
              <span>/</span>
              <span className="text-gray-400">الشركات</span>
              <span>/</span>
              <span className="text-success font-bold">تفاصيل الشركة</span>
           </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="flex justify-between items-start mb-6 border-b border-gray-50 pb-6">
          ...
          <div className="text-right">
             ...
             <p className="text-sm text-gray-500">{company.name_place || "بدون مكان"}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-right">
           <div className="flex items-center justify-end gap-4 p-4 rounded-lg bg-gray-50 border border-gray-100">
              <div>
                 <div className="text-xs text-gray-500 mb-1">المالك</div>
                 <div className="font-bold text-gray-800">{company.name_onwer || "غير محدد"}</div>
              </div>
           ...
           <div className="flex items-center justify-end gap-4 p-4 rounded-lg bg-gray-50 border border-gray-100">
              <div>
                 <div className="text-xs text-gray-500 mb-1">الموقع/العنوان</div>
                 <div className="font-bold text-gray-800">{company.location || "لا يوجد"}</div>
              </div>
              <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-gray-500 shrink-0 border border-gray-100">
           ...
           <div className="flex items-center justify-end gap-4 p-4 rounded-lg bg-gray-50 border border-gray-100">
              <div>
                 ...
                 <div className="font-bold text-gray-800 font-mono">{company.phone || "لا يوجد"}</div>
              </div>
              <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-gray-500 shrink-0 border border-gray-100">
           ...
           <div className="flex items-center justify-end gap-4 p-4 rounded-lg bg-gray-50 border border-gray-100">
              <div>
                 ...
                 <div className="font-bold text-gray-800 font-mono">{company.email || "لا يوجد"}</div>
              </div>
              <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-gray-500 shrink-0 border border-gray-100">
