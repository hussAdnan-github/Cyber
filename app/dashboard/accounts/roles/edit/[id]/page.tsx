...
export default function EditRolePage({ params }: { params: { id: string } }) {
  const [roleName, setRoleName] = useState("انشاء مستخدم"); // Default for demonstration
  
  // Initialize with some active state to match the edit page dummy data
  ...
  const [matrixPermissions, setMatrixPermissions] = useState<Record<string, Record<string, boolean>>>({
    "سجل التدقيق (Audit Logs)": { add: true },
    "الحسابات (Accounts)": { add: true },
  });
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                  ...
                  <div className="space-y-3">
                    {category.items.map((item, i) => (
                      <div key={i} className={`flex justify-between items-center ${pageAccess[item] ? 'bg-white p-2 rounded border border-gray-100' : ''}`}>
                        <Toggle 
                          checked={pageAccess[item] || false} 
                          onChange={() => togglePageAccessItem(item)} 
                        />
                        <span className={`text-xs ${pageAccess[item] ? 'font-bold text-gray-600' : 'text-gray-600'}`}>{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
