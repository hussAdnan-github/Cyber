...
export default function EditRolePage(props: { params: Promise<{ id: string }> }) {
    ...
    const fetchData = async () => {
      try {
        ...
        if (roleData) {
          setFormData({
            name: roleData.name || "",
            permissions_ids: Array.isArray(roleData.permissions) ? roleData.permissions.map((p: any) => p.id || p) : []
          });
        }
      } catch (err) {
        console.error("Error fetching data", err);
        setError("فشل جلب بيانات الدور أو الصلاحيات.");
      } finally {
        setIsFetching(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.patch(`/group/${params.id}/`, formData);
      if (response.data?.success || response.status === 200 || response.status === 204) {
        router.push("/dashboard/accounts/roles");
        router.refresh();
      } else {
        setError(response.data?.message || "حدث خطأ أثناء تعديل الدور");
      }
    } catch (err: any) {
      console.error(err);
      setError(err.response?.data?.message || err.response?.data?.error || "حدث خطأ أثناء تعديل الدور");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h2 className="text-2xl font-bold text-gray-800">تعديل الدور</h2>
      </div>

      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="text-right mb-8 border-b border-gray-100 pb-6">
          <h3 className="text-xl font-bold text-gray-800">تعديل بيانات الدور</h3>
          <p className="text-sm text-gray-500 mt-1">تحديث اسم الدور وصلاحياته</p>
        </div>
