...
export default function AddCompanionPage() {
  ...
  useEffect(() => {
    const fetchDropdowns = async () => {
      try {
        const [guestsRes, nationalitiesRes] = await Promise.all([
  ...
  const onSubmit = async (data: CompanionFormData) => {
    ...
    try {
      ...
      const response = await api.post('/hotal/companions/', formData, {
        headers: {
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:w-72 flex flex-col gap-6 order-1 lg:order-2 shrink-0">
            ...
            <div className="flex gap-2">
              ...
              >
                {isSubmitting ? "جاري الحفظ..." : "حفظ المرافق"}
              </button>
            </div>
            ...
            <div className="bg-[#0f172a] rounded-xl shadow-md p-6 text-white text-right">
              ...
              <p className="text-gray-300 text-sm leading-relaxed">
                يجب أن يكون النزيل الأساسي مسجلاً مسبقاً في النظام ليتم ربط المرافق به.
              </p>
            </div>
