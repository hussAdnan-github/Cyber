...
export default function EditCompanionPage() {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<CompanionFormData>();

  useEffect(() => {
    const fetchData = async () => {
      try {
        const [companionRes, guestsRes, nationalitiesRes] = await Promise.all([
          api.get(`/hotal/companions/${id}/`),
        ...
        if (companionRes.data?.success) {
          const companion = companionRes.data.data;
          
          setExistingPic(companion.pic || null);

          reset({
            name: companion.name || "",
            type_id: companion.type_id ? String(companion.type_id) : ("" as any),
            number_id: companion.number_id || "",
            phone: companion.phone || "",
            phone2: companion.phone2 || "",
            evaluation: companion.evaluation ? String(companion.evaluation) : "",
            person: companion.person || ("" as any),
            nationality: companion.nationality || ("" as any),
          });
        }
      } catch (error) {
        console.error("Error fetching data:", error);
        setSubmitError("فشل في جلب بيانات المرافق");
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex justify-between items-end mb-6 text-right border-b border-gray-100 pb-4">
          <div>
             <h2 className="text-2xl font-bold text-gray-800 mb-1">تعديل بيانات المرافق</h2>
             <p className="text-sm text-gray-500">تحديث بيانات المرافق في النظام.</p>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-right">
                <div>
                  ...
                  <select 
                    ...
                    dir="rtl"
                  >
                    <option value="">اختر النزيل الأساسي...</option>
                    {guests.map(g => (
                      <option key={g.id} value={g.id}>{g.name}</option>
                    ))}
                  </select>
                  {errors.person && <p className="text-red-500 text-xs mt-1">{errors.person.message}</p>}
                </div>
                <div>
